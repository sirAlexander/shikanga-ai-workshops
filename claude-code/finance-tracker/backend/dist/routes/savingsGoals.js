...
router.get('/', async (req, res) => {
    try {
        const goals = await (0, database_1.dbAll)(`
...
router.post('/', async (req, res) => {
    ...
    `, [name, target_amount, current_amount || 0, deadline || null]);
        const newGoal = await (0, database_1.dbGet)(`
...
router.put('/:id', async (req, res) => {
    ...
    `, [name, target_amount, current_amount, deadline, id]);
        const updatedGoal = await (0, database_1.dbGet)(`
      SELECT * FROM savings_goals WHERE id = ?
    `, [id]);
        if (!updatedGoal) {
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await (0, database_1.dbRun)('DELETE FROM savings_goals WHERE id = ?', [id]);
